...
#######################################
	s.vec = M %*% popn.Mat[n-1,]                       ### Matrix multiplication of population trajectory
...
##############################################
	#f.vec	= Scone * Ctree * Cmax * (ad.jv*x[5] + x[6] + Cfec*(ad.jv*x[11] + x[12]))
	f.vec[1]= F.par[5] * (0.5/(1+exp(5*(LAI-2.25))) + 0.5) * F.par[3] * 
				( F.par[2]*s.vec[5] + s.vec[6]   +   F.par[1] * (F.par[2]*s.vec[11] + s.vec[12]) )
	f.vec[2]= s.vec[1] * SR
	f.vec[2]= popn.Mat[n-1,1] * SR

###################################################################
### add f(Ax) the nonlinear function to the surviving population
###################################################################
	popn.Mat[n,]= s.vec + f.vec
...
for (p in 2:stages){ 
  ...
  if (p >= 9) lines(1:(Gen-1), TreeMort.Mat[,p], 'l', xlim=c(0,Gen), lty =2, col=p-6, lwd=1.5)
	...
	Out <- list(popn.Mat, Pop.Total, Prev.v, LAI.v, r.cache.v, SR.v, M, s.vec, f.vec)    ### What pine6 returns
	names(Out) <- c('Pop_Projection', 'Pop_Totals', 'Prevalence', 'LAI', 'r_cache', 'SR', 'Projection_Matrix', 'S', 'f.vec')
	Out
}	# END OF pine6

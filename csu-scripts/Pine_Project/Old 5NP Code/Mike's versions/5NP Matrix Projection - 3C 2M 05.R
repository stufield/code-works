########################################################################
########## 3 CLASS GENETICS MODEL w EPISODIC/PULSE BETA ################
###################### DETERMINISTIC MODEL #############################
########################################################################
# require(popbio)
rm(list=ls(all=TRUE))
...
####################################################################################
########### STORE INITIAL CONDITIONS FOR VARIABLES OF INTEREST #####################
####################################################################################
freq.R = c(rep(0,Gen))
freq.r = c(rep(0,Gen))
...
Beta.vec = c(rep(0,Gen)); Beta.vec[1]= NA
...
#####################################################
  Beta = Mod(0.635*asin(sin((0.16*n)-0.16)))
  Beta.vec[n] = Beta
###########################################################
...
########### OUTPUTS OF INTEREST ###################
###################################################
	freq.R[n] = (2*(sum(popn.Mat[n,1:3])+sum(popn.Mat[n,10:12]))+(sum(popn.Mat[n,4:6])+sum(popn.Mat[n,13:15])))/(sum(popn.Mat[n,])*2)
	freq.r[n] = (2*(sum(popn.Mat[n,7:9])+sum(popn.Mat[n,16:18]))+(sum(popn.Mat[n,4:6])+sum(popn.Mat[n,13:15])))/(sum(popn.Mat[n,])*2)

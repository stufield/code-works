...
### Using the packaged function stuRpkg
Ro.fun(TM,FM)
...
###################################################
### Double Check ##### Formula from paper Fig. 3.
...
### from Oli et al. 2006 ####
#############################
### Pick some values from thin air!
Beta = 0.2; Gamma = 0.1
Ps = 0.9; Pi = 0.5; Pr = 0.7

### T & F for disease (dis) ############
T.dis <- matrix(c(Ps*(1-Beta), 0, 0,
			Ps*Beta, Pi*(1-Gamma), 0,
			0, Pi*Gamma, Pr), 3, 3, byrow=TRUE)

F.dis <- diag(c(0, Ps*Beta, 0))		### use diag function to set up F for disease

### Use above function to calculate Ro for the disease (Infection (I))
RoI.a <- Ro.fun(T.dis, F.dis)
RoI.a

...
#
# Figure widths:
#   Proportial scaling of figure width. Used for studies with 
...
sqs_wrapper <- function(hyb.fig.scale=1, med.fig.scale=1, cal.fig.scale=1,
  ...
  if (! "Adat" %in% names(template.pairs))
    stop("Adat missing from sqs_data.txt")
...
load.template.pairs <- function() {
  ...
  template.pairs.temp = lapply(tokens, function(pair) {
    if (length(pair) != 2)
      stop(sprintf("sqs_data.txt is in an incorrect format. Should contain 'key: value' pairs.\nFound '%s'", paste(pair, collapse=", ")))
    key = gsub("^[\t ]+|[\t ]+$", "", pair[1])
    value= gsub("^[\t ]+|[\t ]+$", "", pair[2])
    c(key, value)
  })
  ...
  if ("AptMenu" %in% names(template.pairs)) {
    if (template.pairs[['AptMenu']] %in% c("Premium", "450plex")) {
      ...
      stop("AptMenu must be either Premium or 450plex. Please check sqs_data.txt")
  }
  else
    stop("AptMenu missing from sqs_data.txt")
  template.pairs
...
write.sqs.data.tex <- function(template.pairs, adat.pairs, adat, sample.type.table) {
  out.file <- file("sqs_data.tex", open="w")
...
create.hyb.norm.data <- function(adat, fig.scale, fig.width, jitter.x, use.log.scale) {
  figure("plots/hyb_norm.pdf", 4.4, 5*fig.width, scale=fig.scale)
  par(mar=c(3.2,4,3,2))
  tab <- plot.scale.factors(adat, hyb=T, scale=fig.scale, as.boxplot=T, jitter.x=jitter.x, use.log.scale=use.log.scale)
  close_figure("plots/hyb_norm.pdf")
  write.latex(tab, row.names="Run", file='tables/hyb_norm.tex')
  ...
  cat(sprintf("\\renewcommand{\\NumHybFail}{%s %i sample%s}\n", were.was, nrow(hyb.fails), plural), file="sqs_data.tex", append=T)
  #cat(sprintf("\\renewcommand{\\NumHybFail}{%i}\n", nrow(hyb.fails)), file="sqs_data.tex", append=T)

  if (nrow(hyb.fails) > 0) {
    cat(sprintf("\\HybFailstrue\n"), file="sqs_data.tex", append=T)
    temp.hyb.fails = hyb.fails
    row.names(temp.hyb.fails) = temp.hyb.fails$ExtIdentifier
    write.latex(temp.hyb.fails[,c("PlateId", "SampleId", "HybControlNormScale")], file='tables/hyb_fail.tex', row.names="ExtIdentifier")
  }
  else
...
create.med.norm.data <- function(adat, fig.scale, legend.scale, fig.width, jitter.x, use.log.scale) {
  ...
  figure("plots/med_norm.pdf", 5, 5*fig.width, scale=fig.scale)
  ...
  close_figure("plots/med_norm.pdf")
  write.latex(tab, row.names="Run: Dilution", file='tables/med_norm.tex')
  ...
  cat(sprintf("\\renewcommand{\\NumMedFail}{%s %i sample%s}\n", were.was, nrow(med.fails), plural), file="sqs_data.tex", append=T)
  cat(sprintf("\\renewcommand{\\NumMedFails}{%i sample%s}\n", nrow(med.fails), plural), file="sqs_data.tex", append=TRUE)
...
create.cal.sop.data <- function(adat, template.pairs, fig.scale, legend.scale) {
  ...
  figure("plots/cal_sop.pdf", 2.9*1.6, 5.8*1.6, scale=fig.scale)
  sop.data <- sop.calibration(adat2, apt.data=apt.data, legend.cex=legend.scale)
  close_figure("plots/cal_sop.pdf")
  ...
  write.latex(sop.data$cal.table, file='tables/cal_sop.tex', row.names="Run")
  write.latex(tail.data, file='tables/tail_apts.tex', row.names="SomaId")
  if (nrow(tail.data) == 1) {
    were.was = "was"
    plural = ""
  }
  ...
  cat(sprintf("\\renewcommand{\\NumCalFail}{%s %i SOMAmer%s}\n", were.was, nrow(tail.data), plural), file='sqs_data.tex', append=T)
  if (nrow(tail.data) > 0) 
...
sample.notes.table <- function(adat){
  ...
  if (all(is.na(adat$SampleNotes))) {
    cat("\\showSampleNotesfalse", file="sqs_data.tex", append=T)
    return()
  }
  ...
  if (nrow(tab) > 0) {
    #names(tab) = "Samples"
    #if (any(is.na(tab$ExtIdentifier))) {
    #row.names(tab) = 1:nrow(tab)
    #rn.name = ""
    #
    #lse {
    #row.names(tab) = as.character(tab$ExtIdentifier)
    #rn.name = "ExtIdentifier"
    #
    #row.names(tab) = as.character(tab$ExtIdentifier)
    #tab = as.data.frame(tab[, -1])

    tab <- tab[ order(tab$SampleId), ]  # reorder (sgf)
    row.names(tab) = 1:nrow(tab)
    ...
    write.latex(tab, row.names="", file='tables/sample_notes.tex')
    cat("\\showSampleNotestrue", file="sqs_data.tex", append=T)
  }
  else
    cat("\\showSampleNotesfalse", file="sqs_data.tex", append=T)

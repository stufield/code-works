...
\newif\ifshowSampleNotes  % Show sample notes table? 
                % This is automatically set by `sqs-data.tex`
                % To over-ride place one of these two flags below by uncommenting.
                % Must be put below \input{sqs-data.tex} in next section
...
\newif\ifuseLogScale  % use log scale for the normalization/calibration factors?
            % Set by sqs-data.tex, can override by placing one of the next two lines after \input{sqs-data.tex}, but
            % note that the plots whould be changed accordingly. The R command `sqs_wrapper(use_log_scale = T|F)` both
            % writes out the correct flag to \input{sqs-data.tex} and generates the corresponding plots
%\useLogScaletrue
%\useLogScalefalse

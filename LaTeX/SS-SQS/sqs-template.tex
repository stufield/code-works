...
\newif\ifshowSampleNotes  % Show sample notes table? 
                % This is automatically set by sqs_data.tex
                % To over-ride place one of these two flags below by uncommenting.
                % Must be put below \input{sqs_data.tex} in next section
...
\newif\ifuseLogScale  % use log scale for the normalization/calibration factors?
            % Set by sqs_data.tex, can override by placing one of the next two lines after \input{sqs_data.tex}, but
            % note that the plots whould be changed accordingly. The R command sqs.wrapper(use.log.scale=T|F) both
            % writes out the correct flag to \input{sqs_data.tex} and generates the corresponding plots
%\useLogScaletrue
%\useLogScalefalse
...
\input{sqs-data.tex} % File created by R code
...
\begin{figure}[H]
  \centering
  \includegraphics[height=.5\textwidth]{plots/hyb_norm.pdf}
...
\begin{figure}[H]
  \centering
  \includegraphics[height=.5\textwidth]{plots/med_norm.pdf}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=.9\textwidth]{plots/cal_sop.pdf}

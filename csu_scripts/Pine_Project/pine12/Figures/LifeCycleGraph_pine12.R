...
###########################################################
CreateNetGraph <- function(dat,                           #
                           arrowcolors= c("green","red"), #
                           legend= FALSE, ...) {          #
###########################################################
  load.pkg(igraph)
  # Create an igraph object from the matrix of weights
  grph <- graph.adjacency(dat, mode= "directed", 
  ...
  } else cat("Edges by Loop number: 0", "\n")

  #   plot    #
  plot(grph, 
       layout= layout.circle,
       vertex.label.degree= label.angles, ...)
  ...
  if (legend){
  	legend(x=-1.75, y=-1, col=c("black", "gray50", "white"),
        ...
        title="Arrows:", bg="lavender", box.lty=0, cex=0.8)
    legend(x=1, y=-1.0, col=c("white","black"),
  	    pch= 19,
        legend= c("Susceptible", "Infected"), 
        title="Nodes:", bg="lavender", box.lty=0, cex=0.9)
  }
}
...
#adjmat = matrix(sample(0:1,25*25, replace=T),ncol=25, dimnames=list(LETTERS[1:25],LETTERS[1:25]))

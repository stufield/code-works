### clear the deck
rm(list=ls(all=TRUE))

### Load PopBio package
require(popbio)

### Create initial population & survivorships for genotypes (L=loop within class; __= -> next class) ####
initial.pop=c(rep(10,9),rep(0,9))

### Newborn Survivorship ###
NRR.L.survive=c(rep(0.4,5)); NRR.survive=c(rep(0.05,5)); NRr.L.survive=c(rep(0.4,5))
NRr.survive=c(rep(0.05,5)); Nrr.L.survive=c(rep(0.4,5)); Nrr.survive=c(rep(0.05,5))
NiRR.L.survive=c(rep(0.4,5)); NiRR.survive=c(rep(0.05,5)); NiRr.L.survive=c(rep(0.4,5))
NiRr.survive=c(rep(0.05,5)); Nirr.L.survive=c(rep(0.4,5)); Nirr.survive=c(rep(0.05,5))
###rnorm(50,0.40,0.10)

### Juvenile Survivorship ###
JRR.L.survive=c(rep(0.5,5)); JRR.survive=c(rep(0.15,5)); JRr.L.survive=c(rep(0.5,5))
JRr.survive=c(rep(0.15,5)); Jrr.L.survive=c(rep(0.5,5)); Jrr.survive=c(rep(0.15,5))
JiRR.L.survive=c(rep(0.5,5)); JiRR.survive=c(rep(0.15,5)); JiRr.L.survive=c(rep(0.5,5))
JiRr.survive=c(rep(0.15,5)); Jirr.L.survive=c(rep(0.5,5)); Jirr.survive=c(rep(0.15,5))
###rnorm(50,0.66,0.20)

### Adult Survivorship ###
ARR.L.survive=c(rep(0.95,5)); ARr.L.survive=c(rep(0.95,5)); Arr.L.survive=c(rep(0.95,5))
AiRR.L.survive=c(rep(0.95,5)); AiRr.L.survive=c(rep(0.95,5)); Airr.L.survive=c(rep(0.95,5))
###runif(50,0.90,0.99)
random.vec=rnorm(50,1.0,0.2)

### Matrix to store population results ############
popn.matrix=matrix(0,10,18, byrow=T)
popn.matrix[1,]=initial.pop; popn.matrix

### Define some initial parameters ############ "L = loop" ##########################

SLnRR=sample(NRR.L.survive,1)      #(01)Survivorship of newborn RR NOT moving to next class
SLinRR=sample(NiRR.L.survive,1)    #(10)Survivorship of infected newborn RR NOT moving to next class
SnRR=sample(NRR.survive,1)         #(19)Survivorship of newborn RR moving to next class
SLjRR=sample(JRR.L.survive,1)      #(20)Survivorship of juvenile RR NOT moving to next class
SinRR=sample(NiRR.survive,1)       #(28)Survivorship of infected newborn RR moving to next class
SLijRR=sample(JiRR.L.survive,1)    #(29)Survivorship of infected juvenile RR NOT moving to next class
SjRR=sample(JRR.survive,1)         #(38)Survivorship of juvenile RR moving to next class
SLaRR=sample(ARR.L.survive,1)      #(39)Survivorship of adult RR
SijRR=sample(JiRR.survive,1)       #(47)Survivorship of infected juvenile RR moving to next class
SLiaRR=sample(AiRR.L.survive,1)    #(48)Survivorship of infected adult RR
SLnRr=sample(NRr.L.survive,1)      #(58)Survivorship of newborn Rr NOT moving to next class
SLinRr=sample(NiRr.L.survive,1)    #(67)Survivorship of infected newborn Rr NOT moving to next class
SnRr=sample(NRr.survive,1)         #(76)Survivorship of newborn Rr moving to next class
SLjRr=sample(JRr.L.survive,1)      #(77)Survivorship of juvenile Rr NOT moving to next class
SinRr=sample(NiRr.survive,1)       #(85)Survivorship of infected newborn Rr moving to next class
SLijRr=sample(JiRr.L.survive,1)    #(86)Survivorship of infected juvenile Rr NOT moving to next class
SjRr=sample(JRr.survive,1)         #(95)Survivorship of juvenile Rr moving to next class
SLaRr=sample(ARr.L.survive,1)      #(96)Survivorship of adult Rr
SijRr=sample(JiRr.survive,1)       #(104)Survivorship of infected juvenile Rr moving to next class
SLiaRr=sample(AiRr.L.survive,1)    #(105)Survivorship of infected adult Rr
SLnrr=sample(Nrr.L.survive,1)      #(115)Survivorship of newborn rr NOT moving to next class
SLinrr=sample(Nirr.L.survive,1)    #(124)Survivorship of infected newborn rr NOT moving to next class
Snrr=sample(Nrr.survive,1)         #(133)Survivorship of newborn rr moving to next class
SLjrr=sample(Jrr.L.survive,1)      #(134)Survivorship of juvenile rr NOT moving to next class
Sinrr=sample(Nirr.survive,1)       #(142)Survivorship of infected newborn rr moving to next class
SLijrr=sample(Jirr.L.survive,1)    #(143)Survivorship of infected juvenile rr NOT moving to next class
Sjrr=sample(Jrr.survive,1)         #(152)Survivorship of juvenile rr moving to next class
SLarr=sample(Arr.L.survive,1)      #(153)Survivorship of adult rr
Sijrr=sample(Jirr.survive,1)       #(161)Survivorship of infected juvenile rr moving to next class
SLiarr=sample(Airr.L.survive,1)    #(162)Survivorship of infected adult rr

############ Fecundities of Infected & Uninfected #######################################

Prop <- function(initial.pop){
    (sum(initial.pop)-(initial.pop[1]+initial.pop[4]+initial.pop[7]+initial.pop[10]+initial.pop[13]+initial.pop[16]))
}
prop.jRR = initial.pop[2]/Prop(initial.pop)
prop.jRr = initial.pop[5]/Prop(initial.pop)
prop.jrr = initial.pop[8]/Prop(initial.pop)
prop.aRR = initial.pop[3]/Prop(initial.pop)
prop.aRr = initial.pop[6]/Prop(initial.pop)
prop.arr = initial.pop[9]/Prop(initial.pop)
prop.ijRR = initial.pop[11]/Prop(initial.pop)
prop.ijRr = initial.pop[14]/Prop(initial.pop)
prop.ijrr = initial.pop[17]/Prop(initial.pop)
prop.iaRR = initial.pop[12]/Prop(initial.pop)
prop.iaRr = initial.pop[15]/Prop(initial.pop)
prop.iarr = initial.pop[18]/Prop(initial.pop)
     
#(02)Probability that a RR juvenile produces a RR newborn
FjRR1=2*((prop.jRR*prop.jRR*1)+(prop.jRR*prop.jRr*0.5)+(prop.jRR*prop.aRR*1)+(prop.jRR*prop.aRr*0.5)+
      (prop.jRR*prop.ijRR*1)+(prop.jRR*prop.ijRr*0.5)+(prop.jRR*prop.iaRR*1)+(prop.jRR*prop.iaRr*0.5))
#(56)Probability that a RR juvenile produces a Rr newborn
FjRR2=2*((prop.jRR*prop.jRr*0.5)+(prop.jRR*prop.jrr*1)+(prop.jRR*prop.aRr*0.5)+(prop.jRR*prop.arr*1)+
      (prop.jRR*prop.ijRr*0.5)+(prop.jRR*prop.ijrr*1)+(prop.jRR*prop.iaRr*0.5)+(prop.jRR*prop.iarr*1))  
#(03)Probability that a RR adult produces a RR newborn
FaRR1=4*((prop.aRR*prop.aRR*1)+(prop.aRR*prop.aRr*0.5)+(prop.aRR*prop.jRR*1)+(prop.aRR*prop.jRr*0.5)+
      (prop.aRR*prop.iaRR*1)+(prop.aRR*prop.iaRr*0.5)+(prop.aRR*prop.ijRR*1)+(prop.aRR*prop.ijRr*0.5))
#(57)Probability that a RR adult produces a Rr newborn
FaRR2=4*((prop.aRR*prop.aRr*0.5)+(prop.aRR*prop.arr*1)+(prop.aRR*prop.jRr*0.5)+(prop.aRR*prop.jrr*1)+
      (prop.aRR*prop.iaRr*0.5)+(prop.aRR*prop.iarr*1)+(prop.aRR*prop.ijRr*0.5)+(prop.aRR*prop.ijrr*1))
#(05)Probability that a Rr juvenile produces a RR newborn
FjRr1=2*((prop.jRr*prop.jRR*0.5)+(prop.jRr*prop.jRr*0.25)+(prop.jRr*prop.aRR*0.5)+(prop.jRr*prop.aRr*0.25)+
      (prop.jRr*prop.ijRR*0.5)+(prop.jRr*prop.ijRr*0.25)+(prop.jRr*prop.iaRR*0.5)+(prop.jRr*prop.iaRr*0.25))
#(59)Probability that a Rr juvenile produces a Rr newborn
FjRr2=2*((prop.jRr*prop.jRr*0.5)+(prop.jRr*prop.jrr*0.5)+(prop.jRr*prop.jRR*0.5)+(prop.jRr*prop.aRr*0.5)+
      (prop.jRr*prop.aRR*0.5)+(prop.jRr*prop.arr*0.5)+(prop.jRr*prop.ijRr*0.5)+(prop.jRr*prop.ijrr*0.5)+
      (prop.jRr*prop.ijRR*0.5)+(prop.jRr*prop.iaRr*0.5)+(prop.jRr*prop.iaRR*0.5)+(prop.jRr*prop.iarr*0.5))
#(113)Probability that a Rr juvenile produces a rr newborn
FjRr3=2*((prop.jRr*prop.jRr*0.25)+(prop.jRr*prop.jrr*0.5)+(prop.jRr*prop.aRr*0.25)+(prop.jRr*prop.arr*0.5)+
      (prop.jRr*prop.ijRr*0.25)+(prop.jRr*prop.ijrr*0.5)+(prop.jRr*prop.iaRr*0.25)+(prop.jRr*prop.iarr*0.5))
#(06)Probability that a Rr adult produces a RR newborn
FaRr1=4*((prop.aRr*prop.aRR*0.5)+(prop.aRr*prop.aRr*0.25)+(prop.aRr*prop.jRR*0.5)+(prop.aRr*prop.jRr*0.25)+
      (prop.aRr*prop.iaRR*0.5)+(prop.aRr*prop.iaRr*0.25)+(prop.aRr*prop.ijRR*0.5)+(prop.aRr*prop.ijRr*0.25))
#(60)Probability that a Rr adult produces a Rr newborn
FaRr2=4*((prop.aRr*prop.aRr*0.5)+(prop.aRr*prop.arr*0.5)+(prop.aRr*prop.aRR*0.5)+(prop.aRr*prop.jRr*0.5)+
      (prop.aRr*prop.jRR*0.5)+(prop.aRr*prop.jrr*0.5)+(prop.aRr*prop.iaRr*0.5)+(prop.aRr*prop.iarr*0.5)+
      (prop.aRr*prop.iaRR*0.5)+(prop.aRr*prop.ijRr*0.5)+(prop.aRr*prop.ijRR*0.5)+(prop.aRr*prop.ijrr*0.5))
#(114)Probability that a Rr adult produces a rr newborn
FaRr3=4*((prop.aRr*prop.aRr*0.25)+(prop.aRr*prop.arr*0.5)+(prop.aRr*prop.jRr*0.25)+(prop.aRr*prop.jrr*0.5)+
      (prop.aRr*prop.iaRr*0.25)+(prop.aRr*prop.iarr*0.5)+(prop.aRr*prop.ijRr*0.25)+(prop.aRr*prop.ijrr*0.5))
#(62)Probability that a rr juvenile produces a Rr newborn
Fjrr1=2*((prop.jrr*prop.jRR*1)+(prop.jrr*prop.jRr*0.5)+(prop.jrr*prop.aRR*1)+(prop.jrr*prop.aRr*0.5)+
      (prop.jrr*prop.ijRR*1)+(prop.jrr*prop.ijRr*0.5)+(prop.jrr*prop.iaRR*1)+(prop.jrr*prop.iaRr*0.5))
#(116)Probability that a rr juvenile produces a rr newborn
Fjrr2=2*((prop.jrr*prop.jrr*1)+(prop.jrr*prop.jRr*0.5)+(prop.jrr*prop.arr*1)+(prop.jrr*prop.aRr*0.5)+
      (prop.jrr*prop.ijrr*1)+(prop.jrr*prop.ijRr*0.5)+(prop.jrr*prop.iarr*1)+(prop.jrr*prop.iaRr*0.5))
#(63)Probability that a rr adult produces a Rr newborn
Farr1=4*((prop.arr*prop.aRR*1)+(prop.arr*prop.aRr*0.5)+(prop.arr*prop.jRR*1)+(prop.arr*prop.jRr*0.5)+
      (prop.arr*prop.iaRR*1)+(prop.arr*prop.iaRr*0.5)+(prop.arr*prop.ijRR*1)+(prop.arr*prop.ijRr*0.5))
#(117)Probability that a rr adult produces a rr newborn
Farr2=4*((prop.arr*prop.arr*1)+(prop.arr*prop.aRr*0.5)+(prop.arr*prop.jrr*1)+(prop.arr*prop.jRr*0.5)+
      (prop.arr*prop.iarr*1)+(prop.arr*prop.iaRr*0.5)+(prop.arr*prop.ijrr*1)+(prop.arr*prop.ijRr*0.5))
################### INFECTED SUBPOP ####################################################
#(11)Probability that a RR infected juvenile produces a RR newborn
FijRR1=2*((prop.ijRR*prop.jRR*1)+(prop.ijRR*prop.jRr*0.5)+(prop.ijRR*prop.aRR*1)+(prop.ijRR*prop.aRr*0.5)+
      (prop.ijRR*prop.ijRR*1)+(prop.ijRR*prop.ijRr*0.5)+(prop.ijRR*prop.iaRR*1)+(prop.ijRR*prop.iaRr*0.5))
#(65)Probability that a RR infected juvenile produces a Rr newborn
FijRR2=2*((prop.ijRR*prop.jRr*0.5)+(prop.ijRR*prop.jrr*1)+(prop.ijRR*prop.aRr*0.5)+(prop.ijRR*prop.arr*1)+
      (prop.ijRR*prop.ijRr*0.5)+(prop.ijRR*prop.ijrr*1)+(prop.ijRR*prop.iaRr*0.5)+(prop.ijRR*prop.iarr*1))
#(12)Probability that a RR infected adult produces a RR newborn
FiaRR1=4*((prop.iaRR*prop.aRR*1)+(prop.iaRR*prop.aRr*0.5)+(prop.iaRR*prop.jRR*1)+(prop.iaRR*prop.jRr*0.5)+
      (prop.iaRR*prop.iaRR*1)+(prop.iaRR*prop.iaRr*0.5)+(prop.iaRR*prop.ijRR*1)+(prop.iaRR*prop.ijRr*0.5))
#(66)Probability that a RR infected adult produces a Rr newborn
FiaRR2=4*((prop.iaRR*prop.aRr*0.5)+(prop.iaRR*prop.arr*1)+(prop.iaRR*prop.jRr*0.5)+(prop.iaRR*prop.jrr*1)+
      (prop.iaRR*prop.iaRr*0.5)+(prop.iaRR*prop.iarr*1)+(prop.iaRR*prop.ijRr*0.5)+(prop.iaRR*prop.ijrr*1))
#(14)Probability that a Rr infected juvenile produces a RR newborn
FijRr1=2*((prop.ijRr*prop.jRR*0.5)+(prop.ijRr*prop.jRr*0.25)+(prop.ijRr*prop.aRR*0.5)+(prop.ijRr*prop.aRr*0.25)+
      (prop.ijRr*prop.ijRR*0.5)+(prop.ijRr*prop.ijRr*0.25)+(prop.ijRr*prop.iaRR*0.5)+(prop.ijRr*prop.iaRr*0.25))
#(68)Probability that a Rr infected juvenile produces a Rr newborn
FijRr2=2*((prop.ijRr*prop.jRr*0.5)+(prop.ijRr*prop.jrr*0.5)+(prop.ijRr*prop.jRR*0.5)+(prop.ijRr*prop.aRr*0.5)+
      (prop.ijRr*prop.aRR*0.5)+(prop.ijRr*prop.arr*0.5)+(prop.ijRr*prop.ijRr*0.5)+(prop.ijRr*prop.ijrr*0.5)+
      (prop.ijRr*prop.ijRR*0.5)+(prop.ijRr*prop.iaRr*0.5)+(prop.ijRr*prop.iaRR*0.5)+(prop.ijRr*prop.iarr*0.5))
#(122)Probability that a Rr infected juvenile produces a rr newborn
FijRr3=2*((prop.ijRr*prop.jRr*0.25)+(prop.ijRr*prop.jrr*0.5)+(prop.ijRr*prop.aRr*0.25)+(prop.ijRr*prop.arr*0.5)+
      (prop.ijRr*prop.ijRr*0.25)+(prop.ijRr*prop.ijrr*0.5)+(prop.ijRr*prop.iaRr*0.25)+(prop.ijRr*prop.iarr*0.5))
#(15)Probability that a Rr infected adult produces a RR newborn
FiaRr1=4*((prop.iaRr*prop.aRR*0.5)+(prop.iaRr*prop.aRr*0.25)+(prop.iaRr*prop.jRR*0.5)+(prop.iaRr*prop.jRr*0.25)+
      (prop.iaRr*prop.iaRR*0.5)+(prop.iaRr*prop.iaRr*0.25)+(prop.iaRr*prop.ijRR*0.5)+(prop.iaRr*prop.ijRr*0.25))
#(69)Probability that a Rr infected adult produces a Rr newborn
FiaRr2=4*((prop.iaRr*prop.aRr*0.5)+(prop.iaRr*prop.arr*0.5)+(prop.iaRr*prop.aRR*0.5)+(prop.iaRr*prop.jRr*0.5)+
      (prop.iaRr*prop.jRR*0.5)+(prop.iaRr*prop.jrr*0.5)+(prop.iaRr*prop.iaRr*0.5)+(prop.iaRr*prop.iarr*0.5)+
      (prop.iaRr*prop.iaRR*0.5)+(prop.iaRr*prop.ijRr*0.5)+(prop.iaRr*prop.ijRR*0.5)+(prop.iaRr*prop.ijrr*0.5))
#(123)Probability that a Rr infected adult produces a rr newborn
FiaRr3=4*((prop.iaRr*prop.aRr*0.25)+(prop.iaRr*prop.arr*0.5)+(prop.iaRr*prop.jRr*0.25)+(prop.iaRr*prop.jrr*0.5)+
      (prop.iaRr*prop.iaRr*0.25)+(prop.iaRr*prop.iarr*0.5)+(prop.iaRr*prop.ijRr*0.25)+(prop.iaRr*prop.ijrr*0.5))
#(71)Probability that a rr infected juvenile produces a Rr newborn
Fijrr1=2*((prop.ijrr*prop.jRR*1)+(prop.ijrr*prop.jRr*0.5)+(prop.ijrr*prop.aRR*1)+(prop.ijrr*prop.aRr*0.5)+
      (prop.ijrr*prop.ijRR*1)+(prop.ijrr*prop.ijRr*0.5)+(prop.ijrr*prop.iaRR*1)+(prop.ijrr*prop.iaRr*0.5))
#(125)Probability that a rr infected juvenile produces a rr newborn
Fijrr2=2*((prop.ijrr*prop.jrr*1)+(prop.ijrr*prop.jRr*0.5)+(prop.ijrr*prop.arr*1)+(prop.ijrr*prop.aRr*0.5)+
      (prop.ijrr*prop.ijrr*1)+(prop.ijrr*prop.ijRr*0.5)+(prop.ijrr*prop.iarr*1)+(prop.ijrr*prop.iaRr*0.5))
#(72)Probability that a rr infected adult produces a Rr newborn
Fiarr1=4*((prop.iarr*prop.aRR*1)+(prop.iarr*prop.aRr*0.5)+(prop.iarr*prop.jRR*1)+(prop.iarr*prop.jRr*0.5)+
      (prop.iarr*prop.iaRR*1)+(prop.iarr*prop.iaRr*0.5)+(prop.iarr*prop.ijRR*1)+(prop.iarr*prop.ijRr*0.5))
#(126)Probability that a rr infected adult produces a rr newborn
Fiarr2=4*((prop.iarr*prop.arr*1)+(prop.iarr*prop.aRr*0.5)+(prop.iarr*prop.jrr*1)+(prop.iarr*prop.jRr*0.5)+
      (prop.iarr*prop.iarr*1)+(prop.iarr*prop.iaRr*0.5)+(prop.iarr*prop.ijrr*1)+(prop.iarr*prop.ijRr*0.5))

#F=(FjRR1+FjRR2+FaRR1+FaRR1+FjRr1+FjRr2+FjRr3+FaRr1+FaRr2+FaRr3+Fjrr1+Fjrr2+Farr1+Farr2);F
#Fi=(FijRR1+FijRR2+FiaRR1+FiaRR1+FijRr1+FijRr2+FijRr3+FiaRr1+FiaRr2+FiaRr3+Fijrr1+Fijrr2+Fiarr1+Fiarr2);Fi

######### Infection Probabilities B & a ####################

B=0.1; a=0.7
#B.nRR= 1; B.nRr= 1; B.nrr= 1; B.jRR= 1; B.jRr= 1; B.jrr= 1; B.aRR= 1; B.aRr= 1; B.arr= 1
#a.nRR= 1; a.nRr= 1; a.nrr= 1; a.jRR= 1; a.jRr= 1; a.jrr= 1; a.aRR= 1; a.aRr= 1; a.arr= 1

################################################
###### Matrix to store parameters ##############
################################################

initial.pars=c(SLnRR*(1-B),FjRR1,FaRR1,0,FjRr1,FaRr1,0,0,0,          SLinRR*a,FijRR1,FiaRR1,0,FijRr1,FiaRr1,0,0,0,
		  SnRR*(1-B),SLjRR*(1-B),0,0,0,0,0,0,0,                 SinRR*a,SLijRR*a,0,0,0,0,0,0,0,
		  0,SjRR*(1-B),SLaRR*(1-B),0,0,0,0,0,0,                 0,SijRR*a,SLiaRR*a,0,0,0,0,0,0,
		  0,FjRR2,FaRR2,SLnRr*(1-B),FjRr2,FaRr2,0,Fjrr1,Farr1,  0,FijRR2,FiaRR2,SLinRr*a,FijRr2,FiaRr2,0,Fijrr1,Fiarr1,
		  0,0,0,SnRr*(1-B),SLjRr*(1-B),0,0,0,0,                 0,0,0,SinRr*a,SLijRr*a,0,0,0,0,
		  0,0,0,0,SjRr*(1-B),SLaRr*(1-B),0,0,0,                 0,0,0,0,SijRr*a,SLiaRr*a,0,0,0,
		  0,0,0,0,FjRr3,FaRr3,SLnrr*(1-B),Fjrr2,Farr2,          0,0,0,0,FijRr3,FiaRr3,SLinrr*a,Fijrr2,Fiarr2,
		  0,0,0,0,0,0,Snrr*(1-B),SLjrr*(1-B),0,                 0,0,0,0,0,0,Sinrr*a,SLijrr*a,0,
		  0,0,0,0,0,0,0,Sjrr*(1-B),SLarr*(1-B),                 0,0,0,0,0,0,0,Sijrr*a,SLiarr*a,
              SLnRR*B,0,0,0,0,0,0,0,0,            SLinRR*(1-a),0,0,0,0,0,0,0,0,
              SnRR*B,SLjRR*B,0,0,0,0,0,0,0,       SinRR*(1-a),SLijRR*(1-a),0,0,0,0,0,0,0,
              0,SjRR*B,SLaRR*B,0,0,0,0,0,0,       0,SijRR*(1-a),SLiaRR*(1-a),0,0,0,0,0,0,
              0,0,0,SLnRr*B,0,0,0,0,0,            0,0,0,SLinRr*(1-a),0,0,0,0,0,
              0,0,0,SnRr*B,SLjRr*B,0,0,0,0,       0,0,0,SinRr*(1-a),SLijRr*(1-a),0,0,0,0,
              0,0,0,0,SjRr*B,SLaRr*B,0,0,0,       0,0,0,0,SijRr*(1-a),SLiaRr*(1-a),0,0,0,
              0,0,0,0,0,0,SLnrr*B,0,0,            0,0,0,0,0,0,SLinrr*(1-a),0,0,
              0,0,0,0,0,0,Snrr*B,SLjrr*B,0,       0,0,0,0,0,0,Sinrr*(1-a),SLijrr*(1-a),0,
              0,0,0,0,0,0,0,Sjrr*B,SLarr*B,       0,0,0,0,0,0,0,Sijrr*(1-a),SLiarr*(1-a))

par.matrix=matrix(0,10,324, byrow=T)
par.matrix[1,]=initial.pars; #par.matrix
X=matrix(initial.pars,18,18,byrow=T);X

##############################################################################
### Creat vectors to store values from population matrix #####################
##############################################################################
lambda.vec=c(rep(0,10)) ### 10 here = n # generations in projection
freq.R=c(rep(0,10))     ### 10 here = n
freq.R[1]=((2*(initial.pop[1]+initial.pop[2]+initial.pop[3]+initial.pop[10]+initial.pop[11]+initial.pop[12]))
     +(initial.pop[4]+initial.pop[5]+initial.pop[6]+initial.pop[13]+initial.pop[14]+initial.pop[15]))/(sum(initial.pop)*2)
freq.r=c(rep(0,10))     ### 10 here = n
freq.r[1]=((2*(initial.pop[7]+initial.pop[8]+initial.pop[9]+initial.pop[16]+initial.pop[17]+initial.pop[18]))
     +(initial.pop[4]+initial.pop[5]+initial.pop[6]+initial.pop[13]+initial.pop[14]+initial.pop[15]))/(sum(initial.pop)*2)
total.pop=c(rep(0,10)); total.pop[1]=sum(initial.pop)
test.vec=c(rep(0,10))   ### 10 here = n = # generations in projection
test.vec[1]=initial.pop[1]

####################################################################################
### Now the loop! (with Matrix M within loop) ######## Generations = 10 ############
####################################################################################
for (n in 2:10) {
     M=matrix(par.matrix[n-1,],18,18, byrow=T)
     popn.matrix[n,] = M%*%popn.matrix[n-1,]
####################################################################################
##### Get proportions of MATING genotypes for probabilities ########################
     prop.jRR = popn.matrix[n-1,2]/(sum(popn.matrix[n-1,])-
(popn.matrix[n-1,1]+popn.matrix[n-1,4]+popn.matrix[n-1,7]+popn.matrix[n-1,10]+popn.matrix[n-1,13]+popn.matrix[n-1,16]))
     prop.jRr = popn.matrix[n-1,5]/(sum(popn.matrix[n-1,])-
(popn.matrix[n-1,1]+popn.matrix[n-1,4]+popn.matrix[n-1,7]+popn.matrix[n-1,10]+popn.matrix[n-1,13]+popn.matrix[n-1,16]))
     prop.jrr = popn.matrix[n-1,8]/(sum(popn.matrix[n-1,])-
(popn.matrix[n-1,1]+popn.matrix[n-1,4]+popn.matrix[n-1,7]+popn.matrix[n-1,10]+popn.matrix[n-1,13]+popn.matrix[n-1,16]))
     prop.aRR = popn.matrix[n-1,3]/(sum(popn.matrix[n-1,])-
(popn.matrix[n-1,1]+popn.matrix[n-1,4]+popn.matrix[n-1,7]+popn.matrix[n-1,10]+popn.matrix[n-1,13]+popn.matrix[n-1,16]))
     prop.aRr = popn.matrix[n-1,6]/(sum(popn.matrix[n-1,])-
(popn.matrix[n-1,1]+popn.matrix[n-1,4]+popn.matrix[n-1,7]+popn.matrix[n-1,10]+popn.matrix[n-1,13]+popn.matrix[n-1,16]))
     prop.arr = popn.matrix[n-1,9]/(sum(popn.matrix[n-1,])-
(popn.matrix[n-1,1]+popn.matrix[n-1,4]+popn.matrix[n-1,7]+popn.matrix[n-1,10]+popn.matrix[n-1,13]+popn.matrix[n-1,16]))
#####
     prop.ijRR = popn.matrix[n-1,11]/(sum(popn.matrix[n-1,])-
(popn.matrix[n-1,1]+popn.matrix[n-1,4]+popn.matrix[n-1,7]+popn.matrix[n-1,10]+popn.matrix[n-1,13]+popn.matrix[n-1,16]))
     prop.ijRr = popn.matrix[n-1,14]/(sum(popn.matrix[n-1,])-
(popn.matrix[n-1,1]+popn.matrix[n-1,4]+popn.matrix[n-1,7]+popn.matrix[n-1,10]+popn.matrix[n-1,13]+popn.matrix[n-1,16]))
     prop.ijrr = popn.matrix[n-1,17]/(sum(popn.matrix[n-1,])-
(popn.matrix[n-1,1]+popn.matrix[n-1,4]+popn.matrix[n-1,7]+popn.matrix[n-1,10]+popn.matrix[n-1,13]+popn.matrix[n-1,16]))
     prop.iaRR = popn.matrix[n-1,12]/(sum(popn.matrix[n-1,])-
(popn.matrix[n-1,1]+popn.matrix[n-1,4]+popn.matrix[n-1,7]+popn.matrix[n-1,10]+popn.matrix[n-1,13]+popn.matrix[n-1,16]))
     prop.iaRr = popn.matrix[n-1,15]/(sum(popn.matrix[n-1,])-
(popn.matrix[n-1,1]+popn.matrix[n-1,4]+popn.matrix[n-1,7]+popn.matrix[n-1,10]+popn.matrix[n-1,13]+popn.matrix[n-1,16]))
     prop.iarr = popn.matrix[n-1,18]/(sum(popn.matrix[n-1,])-
(popn.matrix[n-1,1]+popn.matrix[n-1,4]+popn.matrix[n-1,7]+popn.matrix[n-1,10]+popn.matrix[n-1,13]+popn.matrix[n-1,16]))
####################################################################################
     par.matrix[n,1] = SLnRR*(1-B)   ##B.nRR
     par.matrix[n,2] = 2*((prop.jRR*prop.jRR*1)+(prop.jRR*prop.jRr*0.5)+(prop.jRR*prop.aRR*1)+(prop.jRR*prop.aRr*0.5)+
                      (prop.jRR*prop.ijRR*1)+(prop.jRR*prop.ijRr*0.5)+(prop.jRR*prop.iaRR*1)+(prop.jRR*prop.iaRr*0.5)) #*(sample(random,1))
     par.matrix[n,3] = 4*((prop.aRR*prop.aRR*1)+(prop.aRR*prop.aRr*0.5)+(prop.aRR*prop.jRR*1)+(prop.aRR*prop.jRr*0.5)+
                      (prop.aRR*prop.iaRR*1)+(prop.aRR*prop.iaRr*0.5)+(prop.aRR*prop.ijRR*1)+(prop.aRR*prop.ijRr*0.5)) #*(sample(random,1))
     par.matrix[n,5] = 2*((prop.jRr*prop.jRR*0.5)+(prop.jRr*prop.jRr*0.25)+(prop.jRr*prop.aRR*0.5)+(prop.jRr*prop.aRr*0.25)+
                      (prop.jRr*prop.ijRR*0.5)+(prop.jRr*prop.ijRr*0.25)+(prop.jRr*prop.iaRR*0.5)+(prop.jRr*prop.iaRr*0.25)) #*(sample(random,1))
     par.matrix[n,6] = 4*((prop.aRr*prop.aRR*0.5)+(prop.aRr*prop.aRr*0.25)+(prop.aRr*prop.jRR*0.5)+(prop.aRr*prop.jRr*0.25)+
                      (prop.aRr*prop.iaRR*0.5)+(prop.aRr*prop.iaRr*0.25)+(prop.aRr*prop.ijRR*0.5)+(prop.aRr*prop.ijRr*0.25)) #*(sample(random,1))
     par.matrix[n,10] = SLinRR*a     ##a.nRR
     par.matrix[n,11] = 2*((prop.ijRR*prop.jRR*1)+(prop.ijRR*prop.jRr*0.5)+(prop.ijRR*prop.aRR*1)+(prop.ijRR*prop.aRr*0.5)+
                      (prop.ijRR*prop.ijRR*1)+(prop.ijRR*prop.ijRr*0.5)+(prop.ijRR*prop.iaRR*1)+(prop.ijRR*prop.iaRr*0.5))
     par.matrix[n,12] = 4*((prop.iaRR*prop.aRR*1)+(prop.iaRR*prop.aRr*0.5)+(prop.iaRR*prop.jRR*1)+(prop.iaRR*prop.jRr*0.5)+
                      (prop.iaRR*prop.iaRR*1)+(prop.iaRR*prop.iaRr*0.5)+(prop.iaRR*prop.ijRR*1)+(prop.iaRR*prop.ijRr*0.5))
     par.matrix[n,14] = 2*((prop.ijRr*prop.jRR*0.5)+(prop.ijRr*prop.jRr*0.25)+(prop.ijRr*prop.aRR*0.5)+(prop.ijRr*prop.aRr*0.25)+
                      (prop.ijRr*prop.ijRR*0.5)+(prop.ijRr*prop.ijRr*0.25)+(prop.ijRr*prop.iaRR*0.5)+(prop.ijRr*prop.iaRr*0.25))
     par.matrix[n,15] = 4*((prop.iaRr*prop.aRR*0.5)+(prop.iaRr*prop.aRr*0.25)+(prop.iaRr*prop.jRR*0.5)+(prop.iaRr*prop.jRr*0.25)+
                      (prop.iaRr*prop.iaRR*0.5)+(prop.iaRr*prop.iaRr*0.25)+(prop.iaRr*prop.ijRR*0.5)+(prop.iaRr*prop.ijRr*0.25))
     par.matrix[n,19] = SnRR*(1-B)    ##B.nRR
     par.matrix[n,20] = SLjRR*(1-B)   ##B.jRR
     par.matrix[n,28] = SinRR*a       ##a.nRR
     par.matrix[n,29] = SLijRR*a      ##a.jRR
     par.matrix[n,38] = SjRR*(1-B)    ##B.jRR
     par.matrix[n,39] = SLaRR*(1-B)   ##B.aRR
     par.matrix[n,47] = SijRR*a       ##a.jRR
     par.matrix[n,48] = SLiaRR*a      ##a.aRR
     par.matrix[n,56] = 2*((prop.jRR*prop.jRr*0.5)+(prop.jRR*prop.jrr*1)+(prop.jRR*prop.aRr*0.5)+(prop.jRR*prop.arr*1)+
                      (prop.jRR*prop.ijRr*0.5)+(prop.jRR*prop.ijrr*1)+(prop.jRR*prop.iaRr*0.5)+(prop.jRR*prop.iarr*1)) #*(sample(random,1))
     par.matrix[n,57] = 4*((prop.aRR*prop.aRr*0.5)+(prop.aRR*prop.arr*1)+(prop.aRR*prop.jRr*0.5)+(prop.aRR*prop.jrr*1)+
                      (prop.aRR*prop.iaRr*0.5)+(prop.aRR*prop.iarr*1)+(prop.aRR*prop.ijRr*0.5)+(prop.aRR*prop.ijrr*1)) #*(sample(random,1))
     par.matrix[n,58] = SLnRr*(1-B)   ##B.nRr
     par.matrix[n,59] = 2*((prop.jRr*prop.jRr*0.5)+(prop.jRr*prop.jrr*0.5)+(prop.jRr*prop.jRR*0.5)+(prop.jRr*prop.aRr*0.5)+
                        (prop.jRr*prop.aRR*0.5)+(prop.jRr*prop.arr*0.5)+(prop.jRr*prop.ijRr*0.5)+(prop.jRr*prop.ijrr*0.5)+
                        (prop.jRr*prop.ijRR*0.5)+(prop.jRr*prop.iaRr*0.5)+(prop.jRr*prop.iaRR*0.5)+(prop.jRr*prop.iarr*0.5)) #*(sample(random,1))
     par.matrix[n,60] = 4*((prop.aRr*prop.aRr*0.5)+(prop.aRr*prop.arr*0.5)+(prop.aRr*prop.aRR*0.5)+(prop.aRr*prop.jRr*0.5)+
                        (prop.aRr*prop.jRR*0.5)+(prop.aRr*prop.jrr*0.5)+(prop.aRr*prop.iaRr*0.5)+(prop.aRr*prop.iarr*0.5)+
                        (prop.aRr*prop.iaRR*0.5)+(prop.aRr*prop.ijRr*0.5)+(prop.aRr*prop.ijRR*0.5)+(prop.aRr*prop.ijrr*0.5)) #*(sample(random,1))
     par.matrix[n,62] = 2*((prop.jrr*prop.jRR*1)+(prop.jrr*prop.jRr*0.5)+(prop.jrr*prop.aRR*1)+(prop.jrr*prop.aRr*0.5)+
                      (prop.jrr*prop.ijRR*1)+(prop.jrr*prop.ijRr*0.5)+(prop.jrr*prop.iaRR*1)+(prop.jrr*prop.iaRr*0.5)) #*(sample(random,1))
     par.matrix[n,63] = 4*((prop.arr*prop.aRR*1)+(prop.arr*prop.aRr*0.5)+(prop.arr*prop.jRR*1)+(prop.arr*prop.jRr*0.5)+
                      (prop.arr*prop.iaRR*1)+(prop.arr*prop.iaRr*0.5)+(prop.arr*prop.ijRR*1)+(prop.arr*prop.ijRr*0.5)) #*(sample(random,1))
     par.matrix[n,65] = 2*((prop.ijRR*prop.jRr*0.5)+(prop.ijRR*prop.jrr*1)+(prop.ijRR*prop.aRr*0.5)+(prop.ijRR*prop.arr*1)+
                      (prop.ijRR*prop.ijRr*0.5)+(prop.ijRR*prop.ijrr*1)+(prop.ijRR*prop.iaRr*0.5)+(prop.ijRR*prop.iarr*1))
     par.matrix[n,66] = 4*((prop.iaRR*prop.aRr*0.5)+(prop.iaRR*prop.arr*1)+(prop.iaRR*prop.jRr*0.5)+(prop.iaRR*prop.jrr*1)+
                      (prop.iaRR*prop.iaRr*0.5)+(prop.iaRR*prop.iarr*1)+(prop.iaRR*prop.ijRr*0.5)+(prop.iaRR*prop.ijrr*1))
     par.matrix[n,67] = SLinRr*a      ##a.nRr
     par.matrix[n,68] = 2*((prop.ijRr*prop.jRr*0.5)+(prop.ijRr*prop.jrr*0.5)+(prop.ijRr*prop.jRR*0.5)+(prop.ijRr*prop.aRr*0.5)+
                       (prop.ijRr*prop.aRR*0.5)+(prop.ijRr*prop.arr*0.5)+(prop.ijRr*prop.ijRr*0.5)+(prop.ijRr*prop.ijrr*0.5)+
                       (prop.ijRr*prop.ijRR*0.5)+(prop.ijRr*prop.iaRr*0.5)+(prop.ijRr*prop.iaRR*0.5)+(prop.ijRr*prop.iarr*0.5))
     par.matrix[n,69] = 4*((prop.iaRr*prop.aRr*0.5)+(prop.iaRr*prop.arr*0.5)+(prop.iaRr*prop.aRR*0.5)+(prop.iaRr*prop.jRr*0.5)+
                       (prop.iaRr*prop.jRR*0.5)+(prop.iaRr*prop.jrr*0.5)+(prop.iaRr*prop.iaRr*0.5)+(prop.iaRr*prop.iarr*0.5)+
                       (prop.iaRr*prop.iaRR*0.5)+(prop.iaRr*prop.ijRr*0.5)+(prop.iaRr*prop.ijRR*0.5)+(prop.iaRr*prop.ijrr*0.5))
     par.matrix[n,71] = 2*((prop.ijrr*prop.jRR*1)+(prop.ijrr*prop.jRr*0.5)+(prop.ijrr*prop.aRR*1)+(prop.ijrr*prop.aRr*0.5)+
                      (prop.ijrr*prop.ijRR*1)+(prop.ijrr*prop.ijRr*0.5)+(prop.ijrr*prop.iaRR*1)+(prop.ijrr*prop.iaRr*0.5))
     par.matrix[n,72] = 4*((prop.iarr*prop.aRR*1)+(prop.iarr*prop.aRr*0.5)+(prop.iarr*prop.jRR*1)+(prop.iarr*prop.jRr*0.5)+
                      (prop.iarr*prop.iaRR*1)+(prop.iarr*prop.iaRr*0.5)+(prop.iarr*prop.ijRR*1)+(prop.iarr*prop.ijRr*0.5))
     par.matrix[n,76] = SnRr*(1-B)    ##B.nRr
     par.matrix[n,77] = SLjRr*(1-B)   ##B.jRr
     par.matrix[n,85] = SinRr*a       ##a.nRr
     par.matrix[n,86] = SLijRr*a      ##a.jRr
     par.matrix[n,95] = SjRr*(1-B)    ##B.jRr
     par.matrix[n,96] = SLaRr*(1-B)   ##B.aRr
     par.matrix[n,104] = SijRr*a      ##a.jRr
     par.matrix[n,105] = SLiaRr*a     ##a.aRr
     par.matrix[n,113] = 2*((prop.jRr*prop.jRr*0.25)+(prop.jRr*prop.jrr*0.5)+(prop.jRr*prop.aRr*0.25)+(prop.jRr*prop.arr*0.5)+
                       (prop.jRr*prop.ijRr*0.25)+(prop.jRr*prop.ijrr*0.5)+(prop.jRr*prop.iaRr*0.25)+(prop.jRr*prop.iarr*0.5)) #*(sample(random,1))
     par.matrix[n,114] = 4*((prop.aRr*prop.aRr*0.25)+(prop.aRr*prop.arr*0.5)+(prop.aRr*prop.jRr*0.25)+(prop.aRr*prop.jrr*0.5)+
                       (prop.aRr*prop.iaRr*0.25)+(prop.aRr*prop.iarr*0.5)+(prop.aRr*prop.ijRr*0.25)+(prop.aRr*prop.ijrr*0.5)) #*(sample(random,1))
     par.matrix[n,115] = SLnrr*(1-B)  ##B.nrr
     par.matrix[n,116] = 2*((prop.jrr*prop.jrr*1)+(prop.jrr*prop.jRr*0.5)+(prop.jrr*prop.arr*1)+(prop.jrr*prop.aRr*0.5)+
                       (prop.jrr*prop.ijrr*1)+(prop.jrr*prop.ijRr*0.5)+(prop.jrr*prop.iarr*1)+(prop.jrr*prop.iaRr*0.5)) #*(sample(random,1))
     par.matrix[n,117] = 4*((prop.arr*prop.arr*1)+(prop.arr*prop.aRr*0.5)+(prop.arr*prop.jrr*1)+(prop.arr*prop.jRr*0.5)+
                       (prop.arr*prop.iarr*1)+(prop.arr*prop.iaRr*0.5)+(prop.arr*prop.ijrr*1)+(prop.arr*prop.ijRr*0.5)) #*(sample(random,1))
     par.matrix[n,122] = 2*((prop.ijRr*prop.jRr*0.25)+(prop.ijRr*prop.jrr*0.5)+(prop.ijRr*prop.aRr*0.25)+(prop.ijRr*prop.arr*0.5)+
                       (prop.ijRr*prop.ijRr*0.25)+(prop.ijRr*prop.ijrr*0.5)+(prop.ijRr*prop.iaRr*0.25)+(prop.ijRr*prop.iarr*0.5))
     par.matrix[n,123] = 4*((prop.iaRr*prop.aRr*0.25)+(prop.iaRr*prop.arr*0.5)+(prop.iaRr*prop.jRr*0.25)+(prop.iaRr*prop.jrr*0.5)+
                       (prop.iaRr*prop.iaRr*0.25)+(prop.iaRr*prop.iarr*0.5)+(prop.iaRr*prop.ijRr*0.25)+(prop.iaRr*prop.ijrr*0.5))
     par.matrix[n,124] = SLinrr*a     ##a.nrr
     par.matrix[n,125] = 2*((prop.ijrr*prop.jrr*1)+(prop.ijrr*prop.jRr*0.5)+(prop.ijrr*prop.arr*1)+(prop.ijrr*prop.aRr*0.5)+
                       (prop.ijrr*prop.ijrr*1)+(prop.ijrr*prop.ijRr*0.5)+(prop.ijrr*prop.iarr*1)+(prop.ijrr*prop.iaRr*0.5))
     par.matrix[n,126] = 4*((prop.iarr*prop.arr*1)+(prop.iarr*prop.aRr*0.5)+(prop.iarr*prop.jrr*1)+(prop.iarr*prop.jRr*0.5)+
                       (prop.iarr*prop.iarr*1)+(prop.iarr*prop.iaRr*0.5)+(prop.iarr*prop.ijrr*1)+(prop.iarr*prop.ijRr*0.5))
     par.matrix[n,133] = Snrr*(1-B)   ##B.nrr
     par.matrix[n,134] = SLjrr*(1-B)  ##B.jrr
     par.matrix[n,142] = Sinrr*a      ##a.nrr
     par.matrix[n,143] = SLijrr*a     ##a.jrr
     par.matrix[n,152] = Sjrr*(1-B)   ##B.jrr
     par.matrix[n,153] = SLarr*(1-B)  ##B.arr
     par.matrix[n,161] = Sijrr*a     ##a.jrr
     par.matrix[n,162] = SLiarr*a     ##a.arr
##################### Second half of the subdivided Matrix - No Fecundities; only Survivorship ####################
     par.matrix[n,163] = SLnRR*B      ##B.nRR
     par.matrix[n,172] = SLinRR*(1-a) ##a.nRR
     par.matrix[n,181] = SnRR*B       ##B.nRR
     par.matrix[n,182] = SLjRR*B      ##B.jRR
     par.matrix[n,190] = SinRR*(1-a)  ##a.nRR
     par.matrix[n,191] = SLijRR*(1-a) ##a.jRR
     par.matrix[n,200] = SjRR*B       ##B.jRR
     par.matrix[n,201] = SLaRR*B      ##B.aRR
     par.matrix[n,209] = SijRR*(1-a)  ##a.jRR
     par.matrix[n,210] = SLiaRR*(1-a) ##a.aRR
     par.matrix[n,220] = SLnRr*B      ##B.nRr
     par.matrix[n,229] = SLinRr*(1-a) ##a.nRr
     par.matrix[n,238] = SnRr*B       ##B.nRr
     par.matrix[n,239] = SLjRr*B      ##B.jRr
     par.matrix[n,247] = SinRr*(1-a)  ##a.nRr
     par.matrix[n,248] = SLijRr*(1-a) ##a.jRr
     par.matrix[n,257] = SjRr*B       ##B.jRr
     par.matrix[n,258] = SLaRr*B      ##B.aRr
     par.matrix[n,266] = SijRr*(1-a)  ##a.jRr
     par.matrix[n,267] = SLiaRr*(1-a) ##a.aRr
     par.matrix[n,277] = SLnrr*B      ##B.nrr
     par.matrix[n,286] = SLinrr*(1-a) ##a.nrr
     par.matrix[n,295] = Snrr*B       ##B.nrr
     par.matrix[n,296] = SLjrr*B      ##B.jrr
     par.matrix[n,304] = Sinrr*(1-a)  ##a.nrr
     par.matrix[n,305] = SLijrr*(1-a) ##a.jrr
     par.matrix[n,314] = Sjrr*B       ##B.jrr
     par.matrix[n,315] = SLarr*B      ##B.arr
     par.matrix[n,323] = Sijrr*(1-a) ##a.jrr
     par.matrix[n,324] = SLiarr*(1-a) ##a.arr
##### Allelic Frequencies #########################
freq.R[n]=((2*(popn.matrix[n,1]+popn.matrix[n,2]+popn.matrix[n,3]+popn.matrix[n,10]+popn.matrix[n,11]+popn.matrix[n,12]))
          +(popn.matrix[n,4]+popn.matrix[n,5]+popn.matrix[n,6]+popn.matrix[n,13]+popn.matrix[n,14]+popn.matrix[n,15]))/(sum(popn.matrix[n,])*2)
freq.r[n]=((2*(popn.matrix[n,7]+popn.matrix[n,8]+popn.matrix[n,9]+popn.matrix[n,16]+popn.matrix[n,17]+popn.matrix[n,18]))
          +(popn.matrix[n,4]+popn.matrix[n,5]+popn.matrix[n,6]+popn.matrix[n,13]+popn.matrix[n,14]+popn.matrix[n,15]))/(sum(popn.matrix[n,])*2)
     lambda.vec[n-1] = eigen.analysis(M)$lambda
     total.pop[n]= sum(popn.matrix[n,])
     test.vec[n] = popn.matrix[n,n]
}
### Matrix out of loop for Lambda of last generation ###
M=matrix(par.matrix[n,],18,18, byrow=T)  
lambda.vec[n] = eigen.analysis(M)$lambda


#######################################
##### Outputs of Interest #############
#######################################
par.matrix;
popn.matrix;
test.vec;
lambda.vec;
freq.R; freq.r
Mating.props=c(rep(0,12)); Mating.props[1]=prop.jRR; Mating.props[2]=prop.jRr; Mating.props[3]=prop.jrr;
Mating.props[4]=prop.aRR; Mating.props[5]=prop.aRr; Mating.props[6]=prop.arr; Mating.props[7]=prop.ijRR;
Mating.props[8]=prop.ijRr; Mating.props[9]=prop.ijrr; Mating.props[10]=prop.iaRR; Mating.props[11]=prop.iaRr;
Mating.props[12]=prop.iarr; Mating.props

#######################################################################
### Whitebark Pine Population Variables by Generation #################
#######################################################################
par(mfrow=c(1,3))
plot(1:n, total.pop,'l', xlab="Year", ylab="Total Whitebark Pine Population", col='dark red', lwd=2)
plot(1:10, lambda.vec, 'l', xlab="Year", ylab="Matrix Lambda", col='dark red', lwd=2)
pop.diffs=c(sum(popn.matrix[2,])-sum(popn.matrix[1,]),sum(popn.matrix[3,])-sum(popn.matrix[2,]),sum(popn.matrix[4,])-sum(popn.matrix[3,]),
sum(popn.matrix[5,])-sum(popn.matrix[4,]),sum(popn.matrix[6,])-sum(popn.matrix[5,]),sum(popn.matrix[7,])-sum(popn.matrix[6,]),
sum(popn.matrix[8,])-sum(popn.matrix[7,]),sum(popn.matrix[9,])-sum(popn.matrix[8,]),sum(popn.matrix[10,])-sum(popn.matrix[9,]))
plot(1:9, pop.diffs, 'l', xlab="1-9", ylab="Generation Size Difference", col='dark red', lwd=2)

####################################################
### Plot some population graphs ####################
####################################################
par(mfrow=c(2,3))
plot(1:n, popn.matrix[,1],'l', ylim=range(0,40), xlab="Year",ylab="Whitebark Pine Population RR",col='dark red',lwd=2)
lines(1:n, popn.matrix[,2], 'l', col='dark blue',lwd=2)
lines(1:n, popn.matrix[,3], 'l', col='dark green',lwd=2)
text(4,35,'Red = Newborns'); text(4,33,'Blue = Juveniles'); text(4,31,'Green = Adults')
plot(1:n, popn.matrix[,4],'l', ylim=range(0,40), xlab="Year",ylab="Whitebark Pine Population Rr", col='dark red',lwd=2)
lines(1:n, popn.matrix[,5], 'l', col='dark blue',lwd=2)
lines(1:n, popn.matrix[,6], 'l', col='dark green',lwd=2)
text(3,39,'Red = Newborns'); text(3,37,'Blue = Juveniles'); text(3,35,'Green = Adults')
plot(1:n, popn.matrix[,7],'l', ylim=range(0,40), xlab="Year",ylab="Whitebark Pine Population rr", col='dark red',lwd=2)
lines(1:n, popn.matrix[,8], 'l', col='dark blue',lwd=2)
lines(1:n, popn.matrix[,9], 'l', col='dark green',lwd=2)
text(4,35,'Red = Newborns'); text(4,33,'Blue = Juveniles'); text(4,31,'Green = Adults')
########## Infected Subpop ####################
plot(1:n, popn.matrix[,10],'l', ylim=range(0,40), xlab="Year",ylab="Whitebark Pine Population Infected RR",col='dark red',lwd=2)
lines(1:n, popn.matrix[,11], 'l', col='dark blue',lwd=2)
lines(1:n, popn.matrix[,12], 'l', col='dark green',lwd=2)
text(4,35,'Red = Newborns'); text(4,33,'Blue = Juveniles'); text(4,31,'Green = Adults')
plot(1:n, popn.matrix[,13],'l', ylim=range(0,40), xlab="Year",ylab="Whitebark Pine Population Infected Rr", col='dark red',lwd=2)
lines(1:n, popn.matrix[,14], 'l', col='dark blue',lwd=2)
lines(1:n, popn.matrix[,15], 'l', col='dark green',lwd=2)
text(3,39,'Red = Newborns'); text(3,37,'Blue = Juveniles'); text(3,35,'Green = Adults')
plot(1:n, popn.matrix[,16],'l', ylim=range(0,40), xlab="Year",ylab="Whitebark Pine Population Infected rr", col='dark red',lwd=2)
lines(1:n, popn.matrix[,17], 'l', col='dark blue',lwd=2)
lines(1:n, popn.matrix[,18], 'l', col='dark green',lwd=2)
text(4,35,'Red = Newborns'); text(4,33,'Blue = Juveniles'); text(4,31,'Green = Adults')



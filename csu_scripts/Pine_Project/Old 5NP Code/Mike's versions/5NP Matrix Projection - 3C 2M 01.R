...
################ Tolerance Model of Resistance #########################
###### & optional density-dependent fecundity function #################
########################################################################
# require(popbio)
rm(list=ls())
...
NRR=100;	JRR=100;	ARR=100
NRr=1800;	JRr=1800;	ARr=1800
Nrr=8100;	Jrr=8100;	Arr=8100
...
DDF = F                              # Will thie show density-dependent fecundity?
...
###########################################################
  ...
  M.SI = matrix(par.vec.SI,stages,stages,byrow=T); M.SI
	#lambda.vec[c]= eigen.analysis(M.SI)$lambda
####################################################################################
...
##########################################################################
  FA = par.data$Value[6]
  if (DDF) FA = (0.4*exp(-0.00125*(total.pop[n-1]))+0.035)
  FJ = FA*0.5; FAi = FA*F.sel; FJi = FAi*0.5
...
par(mfrow=c(1,3))
plot(1:Gen, Totals.Mat[1:Gen,5], 'l', lty=2, ylim=c(0,max(Totals.Mat[,5])), xlim=c(0,Gen), xlab="Years", main="Total Population", ylab="Total Whitebark Population", col='black', lwd=1.5)
...
plot(1:Gen, Totals.Mat[1:Gen,3],'l', lty=2, ylim=range(0:1.0), ylab="Allele Frequency (R) in Population", main="Frequency R",
   ...
   legend('topleft', cex=0.75, legend=c('Beta= 0.0','Beta= 0.016','Beta= 0.047','Beta= 0.078','Beta= 0.11','Beta= 0.14'),

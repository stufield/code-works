########################################################################
########## 3 CLASS GENETICS MODEL w CATASTROPHE MODEL BETA #############
############### STOCHASTIC MODEL w/ Rounding ###########################
########################################################################
rm(list=ls())
...
P.Prev = c(rep(0,Gen)); P.Prev[1] = sum(I.pop[10:18])/sum(I.pop)   # Parasite Prevalence
Beta.vec = c(rep(0,Gen))
Catastrophe.vec = c(1:8)
Random.Beta.1 = seq(0, 0.65, by=0.05)
Random.Beta.2 = seq(0.70,1.0, by=0.1)
...
for (n in 2:Gen) {
#########################################################
######### Define infection rate Beta  ###################
#########################################################
  C = sample(Catastrophe.vec, 1, replace=F)
  if (C != 8) Beta = sample(Random.Beta.1, 1, replace=F)
  if (C == 8) Beta = sample(Random.Beta.2, 1, replace=F)
  Beta.vec[n] = Beta
###########################################################
...
###################################################
	...
	popn.Mat[which(popn.Mat<1)]= 0               # Reset all individual class values < 1 equal to 0 (no partial individuals)
     popn.Mat[n,] = round(popn.Mat[n,])           # Round all values to have whole individuals
...
###################################################
	...
	freq.R[n] = round(freq.R[n],2)
	freq.r[n] = (2*(sum(popn.Mat[n,7:9])+sum(popn.Mat[n,16:18]))+
			(sum(popn.Mat[n,4:6])+sum(popn.Mat[n,13:15])))/(sum(popn.Mat[n,])*2)
	freq.r[n] = round(freq.r[n],2)
	P.Prev[n] = sum(popn.Mat[n,10:18])/sum(popn.Mat[n,])
	P.Prev[n] = round(P.Prev[n],3)
	total.pop[n]= sum(popn.Mat[n,])
	
}  # END OF GENERATION LOOP

Totals.Mat <- cbind(1:Gen, Beta.vec, freq.R, P.Prev, total.pop, popn.Mat)
colnames(Totals.Mat) = c("Gen","Beta","p(R)","Prevalence","Total Pop","NRR","JRR","ARR","NRr","JRr","ARr",
                          "Nrr","Jrr","Arr","NRRi","JRRi","ARRi","NRri","JRri","ARri","Nrri","Jrri","Arri")
Totals.Mat

...
#################################################################
	...
	Pj = pro(Mate.pop,1);		Hj = pro(Mate.pop,3);		Qj = pro(Mate.pop,5)     # get proportions of each class within the mating popn
	Pa = pro(Mate.pop,2);		Ha = pro(Mate.pop,4);		Qa = pro(Mate.pop,6)     # P = RR; H = Rr; Q = rr; juvie or adult
...
#### Fecundity separate from Survivorship #####################################
  par.vec.F = c(rep(0, stages^2))             # create vector to store results of matings for input into Fecundity matrix (FM)
   ...
   ##### Row 1 of the Fecundity matrix ################################################
	par.vec.F[2]= FJ*((Pj*Pj)+(Pj*Pa)+(Pj*Hj*0.5)+(Pj*Ha*0.5))/Sum.Mpop     # Contribution of JRR to NRR
	par.vec.F[3]= FA*((Pa*Pj)+(Pa*Pa)+(Pa*Hj*0.5)+(Pa*Ha*0.5))/Sum.Mpop     # Contribution of ARR to NRR
	par.vec.F[5]= FJ*((Hj*Pj*0.5)+(Hj*Pa*0.5)+(Hj*Hj*0.25)+(Hj*Ha*0.25))/Sum.Mpop    # Contribution of JRr to NRR
	par.vec.F[6]= FA*((Ha*Pj*0.5)+(Ha*Pa*0.5)+(Ha*Hj*0.25)+(Ha*Ha*0.25))/Sum.Mpop    # Contribution of ARr to NRR
	
  ##### Row 4 of the Fecundity matrix ################################################
	par.vec.F[29]= FJ*((Pj*Qj)+(Pj*Qa)+(Pj*Hj*0.5)+(Pj*Ha*0.5))/Sum.Mpop    # Contribution of JRR to NRr
	par.vec.F[30]= FA*((Pa*Qj)+(Pa*Qa)+(Pa*Hj*0.5)+(Pa*Ha*0.5))/Sum.Mpop    # Contribution of ARR to NRr
	par.vec.F[32]= FJ*((Hj*Hj*0.5)+(Hj*Ha*0.5)+(Hj*Pj*0.5)+(Hj*Pa*0.5)+(Hj*Qj*0.5)+(Hj*Qa*0.5))/Sum.Mpop    # Contribution of JRr to NRr
	par.vec.F[33]= FA*((Ha*Hj*0.5)+(Ha*Ha*0.5)+(Ha*Pj*0.5)+(Ha*Pa*0.5)+(Ha*Qj*0.5)+(Ha*Qa*0.5))/Sum.Mpop    # Contribution of ARr to NRr
	par.vec.F[35]= FJ*((Qj*Pj)+(Qj*Pa)+(Qj*Hj*0.5)+(Qj*Ha*0.5))/Sum.Mpop    # Contribution of Jrr to NRr
	par.vec.F[36]= FA*((Qa*Pj)+(Qa*Pa)+(Qa*Hj*0.5)+(Qa*Ha*0.5))/Sum.Mpop    # Contribution of Arr to NRr
	
  ##### Row 7 of the Fecundity matrix ################################################	
  par.vec.F[59]= FJ*((Hj*Qj*0.5)+(Hj*Qa*0.5)+(Hj*Hj*0.25)+(Hj*Ha*0.25))/Sum.Mpop    # Contribution of JRr to Nrr
	par.vec.F[60]= FA*((Ha*Qj*0.5)+(Ha*Qa*0.5)+(Ha*Hj*0.25)+(Ha*Ha*0.25))/Sum.Mpop    # Contribution of ARr to Nrr
	par.vec.F[62]= FJ*((Qj*Qj)+(Qj*Qa)+(Qj*Hj*0.5)+(Qj*Ha*0.5))/Sum.Mpop     # Contribution of Jrr to Nrr
	par.vec.F[63]= FA*((Qa*Qj)+(Qa*Qa)+(Qa*Hj*0.5)+(Qa*Ha*0.5))/Sum.Mpop     # Contribution of Arr to Nrr

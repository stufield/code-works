...
early.set <- as.numeric(data$Date[early])
late      <- median(which(as.numeric(data$Sunset)==max(as.numeric(data$Sunset))))
late.set  <- as.numeric(data$Date[late])

plot(data$Date, data$Sunset, ylab="Sunset", xlab="Day", type="s",
     col="darkred", lwd=1.5, yaxt="n")

abline(v=late.set, col="darkgreen", lwd=1.5, lty=2)
abline(v=early.set, col="navy", lwd=1.5, lty=2)
legend("topleft", legend=c(format(late.set),format(early.set)), bg="gray90",
